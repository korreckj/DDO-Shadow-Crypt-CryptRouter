...
"""
    ...
    while True:
        ...
        while firstIteration:
            firstTimer = input(
                "Is this your first time using Crypt-Router? type 'yes' or 'y' if so & 'no' or 'n' if not, or press enter to exit.\n" + orange(
                    "Enter Here: ")).lower()
        ...
        while True:
            if not foundInstance:
                userKnowInstance = input(
                    "\nDo you know which instance you are in? type 'yes' or 'y' if so & 'no' or 'n' if not, or press enter to exit\n" + orange(
                        "Enter Response: ")).lower()  # primary question for this while loop
                ...
                wasUnsure = False
                if yesNoAssociation(
                        userKnowInstance):  # checks to see if the response is a valid affirmative/negative response
                        ...
                        if userKnowInstance in yesSet:  # basically, cannot proceed if they do not know their instance.
                            ...
                            userInstance = input(
                                "\nWhich instance are you in? (r/g/b/red/green/blue) or press enter to exit\n" + orange(
                                    "Enter Response: ")).lower()  # primary questions for this while loop
                            if instanceAssociation(
                                    userInstance):  # verifies if any of the user's responses correlate to valid instances
                            ...
                            userDD = input(
                                "\nDo you Have Death's Door charges or Dimension Door Spell? type 'yes' or 'y' if so & 'no' or 'n' if not, or press enter to exit\n" + orange(
                                    "Enter Response: ")).lower()  # allows for alternative to check instance ( returns player to starting room )
                            if (yesNoAssociation(userDD) and userDD in yesSet):
                                flagDD = True
                                print(
                                    "Please use Deaths Door/Dimension door to return to the starting room, and go east to learn which instance you are in.\n")
                                break
            ...
            while True:
                if (inInstance1):
                    ...
                    roomsInInstance1 = ["s", "gc", "rc", "bc", "ym", "gm", "bm", "yp", "yd", "gd", "rd", "ru", "bu",
                                        "r"]
                    ...
                    while True:
                        ...
                        while True:  # dialogue loop for starting room
                            raw = input(
                                "\nWhich of these rooms are you currently in? (or press enter to exit)\nValid inputs: i.e. gc, rc, yellow maze, etc. s/start for starting room, r/ramp for final room <case insensitive>\n" + orange(
                                    "Enter Room: ")).lower()  # collect input
                            ...
                            if startingRoomString in abrvSet:  # easy element check from set copy of map-form list
                                startingRoom = stringToObjMap.get(
                                    startingRoomString)  # because room is mapped to string, easy room retrieval
                        ...
                        while True:  # dialogue loop for destination room
                            ...
                            if destinationRoomString in abrvSet:  # easy element check from set copy of map-form list
                                destinationRoom = stringToObjMap.get(
                                    destinationRoomString)  # because room is mapped to string, easy room retrieval
                                break  # break dialogue loop, no more need to continue this query
                            elif isExitChar(raw):  # check raw input to see if it is the exit character
                                sys.exit()
                            else:
                                print("invalid room, please try again\n")  # inform the user of the error of their ways
                                continue  # restart the while loop
                        path = prioBFS(instance1, startingRoom,
                                       destinationRoom)  # perform priority BFS, store path as a list
                        print("\nHere is your path from " + str(startingRoom) + " to " + str(
                    ...
                    firstIteration = False
                    # populate a list with the valid map-form string identifiers of the room in given instance
                    roomsInInstance2 = ["s", "gc", "bc", "gm", "rm", "bm", "yp", "gp", "rp", "yd", "bd", "yu", "ru",
                                        "r"]
                    ...
                    while True:
                        ...
                        while True:  # dialogue loop for destination room
                            ...
                            if destinationRoomString in abrvSet:  # easy element check from set copy of map-form list
                                ...
                                print("invalid room, please try again\n")  # inform the user of the error of their ways
                                continue  # restart the while loop
                        path = prioBFS(instance2, startingRoom,
                                       destinationRoom)  # perform priority BFS, store path as a list
                        print("\nHere is your path from " + str(startingRoom) + " to " + str(
                            destinationRoom) + "\n")  # reiterate to the user their chosen rooms
                        printPath(path)  # display path to user
                    ...
                    firstIteration = False
                    # populate a list with the valid map-form string identifiers of the room in given instance
                    roomsInInstance3 = ["s", "yc", "rc", "ym", "bm", "gp", "rp", "bp", "gd", "rd", "yu", "gu", "bu",
                                        "r", "gc"]
                    ...
                    while True:
                        ...
                        while True:  # dialogue loop for destination room
                            ...
                            if destinationRoomString in abrvSet:  # easy element check from set copy of map-form list
                                ...
                                print("invalid room, please try again\n")  # inform the user of the error of their ways
                                continue  # restart the while loop
                        path = prioBFS(instance3, startingRoom,
                                       destinationRoom)  # perform priority BFS, store path as a list
                        print("\nHere is your path from " + str(startingRoom) + " to " + str(
                            destinationRoom) + "\n")  # reiterate to the user their chosen rooms
                        printPath(path)  # display path to user
